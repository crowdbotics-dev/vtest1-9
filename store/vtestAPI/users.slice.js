...
export const api_v1_user_list = createAsyncThunk("users/api_v1_user_list", async payload => {
  const response = await apiService.api_v1_user_list(payload);
  return response.data;
});
export const api_v1_user_create = createAsyncThunk("users/api_v1_user_create", async payload => {
  const response = await apiService.api_v1_user_create(payload);
  return response.data;
});
export const api_v1_user_retrieve = createAsyncThunk("users/api_v1_user_retrieve", async payload => {
  const response = await apiService.api_v1_user_retrieve(payload);
  return response.data;
});
export const api_v1_user_update = createAsyncThunk("users/api_v1_user_update", async payload => {
  const response = await apiService.api_v1_user_update(payload);
  return response.data;
});
export const api_v1_user_partial_update = createAsyncThunk("users/api_v1_user_partial_update", async payload => {
  const response = await apiService.api_v1_user_partial_update(payload);
  return response.data;
});
export const api_v1_user_destroy = createAsyncThunk("users/api_v1_user_destroy", async payload => {
  const response = await apiService.api_v1_user_destroy(payload);
  return response.data;
});
const initialState = {
  entities: [],
...
const usersSlice = createSlice({
  ...
  extraReducers: {
    ...
    [api_v1_user_retrieve.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [api_v1_user_retrieve.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [...state.entities.filter(record => record.id !== action.payload.id), action.payload];
        state.api.loading = "idle";
      }
    },
    [api_v1_user_retrieve.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
    ...
    [api_v1_user_destroy.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [api_v1_user_destroy.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.filter(record => record.id !== action.meta.arg?.id);
        state.api.loading = "idle";
      }
    },
    [api_v1_user_destroy.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
